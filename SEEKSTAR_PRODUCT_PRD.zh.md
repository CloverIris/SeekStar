...
### 3.3.1 连续尺度地形，不是盒子套盒子

SeekStar 的 L0、L1、L2、L3 不应该被理解成一层层互相隔离的盒子。它们是同一片语义地形在不同焦距下的表现。

当用户在 L1 看到“汽车”和“飞机”两个相邻区域，放大“汽车”进入 L2 后，看到的应该主要是汽车的结构、来源方向、零件、产业和资料类型。但如果用户在 L2 继续朝着原来 L1 中“飞机”所在的方向移动，L2 的内容应该逐渐混入航空材料、飞机结构、航空资料源和相关来源方向。继续移动后，用户看到的就应该是飞机在 L2 尺度下的细节。

当用户此时拉远回到 L1，SeekStar 不应该把用户带回最初“汽车”盒子的入口，而应该让用户从当前位置附近的“飞机”区域浮出。这才像望远镜或显微镜：镜头移动的是同一片世界，焦距改变的是观察尺度，而不是进入一个被隔离的新时空。

因此，普通的 L0/L1/L2/L3 缩放和横向漫游不创建新的孤儿宇宙，也不新开平行 tab。它们在同一个 scene、同一个坐标系、同一个语义地形里工作。AI Cartographer 的任务是在当前焦点、当前视口、邻近上层节点和移动方向的约束下补足地形，而不是每深入一层就重建一套互不相干的地图。

只有真正失去父级语义位置的入口，才是孤儿层或递归 seed：例如浏览器中点击的外部超链接、直接粘贴的 URL、独立文件、Deep Lens 中被提升成新世界的词语或段落。这些入口需要 AI 向上补全它属于什么领域、主题和来源方向。普通望远镜漫游则应该写回原来的连续地形，让上层地图随用户的真实探索位置变得更完整。
...
L0 是领域层，也是 seed pool，但它不是设置词库的直接列表。

这里展示的是 AI Cartographer 生成并通过结构校验的可探索领域、宏观方向、知识大类、未知边缘和可递归 seed。设置里的预制领域词库只是 opening sky 的高质量提示，类似新闻网站里的“科技 / 财经 / 文化 / 社会”频道名，用来帮助 AI 定向，而不是直接塞进画布的默认节点。

默认 `domain_hint_mode` 是 `guided`：启用的领域词会进入 prompt context，但 AI 仍然要自由穷举、随机启发、生成新的邻域。`pure_ai` 则完全不传预制领域，让 AI 自由决定今晚星空。用户自己输入的 seed、从 Deep Lens 选中的词、从浏览器链接打开的孤儿页，都不再触发默认星空机制，而是围绕那个具体 seed 生成新的语义宇宙。
...
这里必须区分“候选”和“tile”。AI 提出的 source candidate 只是验证队列里的轻量对象，不是真正的 Tile Field 表面。它可以显示在状态提示、Source review、recovery queue 或右侧 AI Map Control 中，但不能在主画布伪装成已经可浏览、可吸附、可 Deep Lens 的 tile。

真正的 L3 tile 只能来自真实观察：DataService/Playwright 成功加载网页、PDF、图片或文档，提取到标题、URL、内容类型、可见文本、截图/缩略图或 source snapshot 后，才创建 source-backed tile。失败的 URL 不变成破碎 tile，而是留在恢复队列里等待重试、替换或作为新 seed 打开。
...
SeekStar 的产品路径只使用真实 AI provider。默认 provider 是 DeepSeek 的 OpenAI-compatible 接口；设置支持真实 API Key 输入，也可以回退读取环境变量。没有 key 时系统应该明确失败并提示配置问题，而不是生成假星图。测试可以使用本地 fixture，但 fixture 不能成为公开 provider、CLI provider、Settings route 或主画布内容来源。
...
AI source candidate 不属于真正 tile 类型。它是候选队列对象，可以在状态层和右侧 Source review 中出现，但不能在主画布上以 tile 的形态假装已经被验证。候选只有在 DataService/Playwright 成功观察后，才进入上述 tile 类型。

主画布只展示 source-backed 内容的清晰状态。失败内容应进入恢复队列或诊断区，不应作为破损 tile 干扰探索。
...
- live：正在由 WebContentsView 或文档表面承载真实内容。

AI 候选和验证失败不是主画布 tile surface 的状态，而是 Source review / recovery queue 的状态。这样用户不会把 AI 的可能性误认为已经存在且已经被验证的网页或文档。
