...
SeekStar 的层级不是无限盒子套盒子，而是同一个有限知识世界在不同尺度下的观察。缩放改变的是比例尺、密度和可见结构，不是把用户传送到一个完全无关的新世界。

上层语义盘面可以理解为被放大的 App 图标：用户聚焦某个领域、主题或来源方向后，这个对象逐渐成为当前屏幕的语义背景，并把内部内容显现出来。它仍然是同一张地图的不同尺度投影，而不是普通文件夹层级。

L1/L2/L3 横向探索后再拉远，系统应该从当前位置附近浮出上层语义，保留 trail、backlink 和 source context。一个可用类比是 Minecraft 的主世界/地狱比例映射：低层尺度里的移动，回到高层尺度时仍应能映射成可理解的路径痕迹。

当前主流程只保留一个 active observation cone，因为用户只有一个主视口。多个概念锥体只用于解释下层高密度信息如何压缩为上层语义对象，不意味着主界面要同时维护多个活跃视口。

信息结构概念说明见 [INFORMATION_STRUCTURE.zh.md](INFORMATION_STRUCTURE.zh.md)，概念图说明见 [design/information _structure/README.md](<design/information _structure/README.md>)。
...
默认同屏最多 25 个 tile 分片。这个限制指当前镜头/视锥的可见容量，不表示周边没有更多内容。

横向移动时，系统可以从缓存池、预渲染和按需加载中继续提供周边区域；如果用户移动速度击穿预加载和缓存，界面可以出现带阻尼感的区域加载边界。加载完成后，这个边界继续向用户开放。

继续向下缩放时，可见 tile 数量应减少，并最终进入浏览器吸附模式。继续向上拉远时，也必须保持密度限制，避免把下层大量真实内容直接倾倒到上层语义盘面。
...
Deep Lens 不是宏观星图继续向下贴近同一张地图，而是从一个真实 tile 打开的独立微观阅读层。它服务于文档内部结构、段落、字符串、词语和字符，不需要和 L0/L1/L2/L3 的宏观空间布局保持几何连续。
