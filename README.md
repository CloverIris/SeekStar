...
当前重点已经进入核心开发阶段：先固定望远镜/星图的 12Level 语义脊柱与五模块边界，再逐步接入更强的数据、AI 与存储实现。

* Electron 桌面宿主；
* React shell 与 PixiJS 星图呈现；
* Vite 开发与构建工具；
* TypeScript 类型系统；
* 2.5D 认知画布与 12Level 语义缩放；
...
* AI Service 与 Storage/Cache Service 边界；
* 暗色蓝调 UI 风格；
* 基础文档与架构约束。
...
* canonical spine 是 L0-L11 12Level；
* seed scene、source terrain、Pixi projection 与 tab runtime 都必须沿同一层级模型工作；
* Playwright Scout 只返回 observation / source snapshot，不直接写事实；
* AI 输出必须经 AI Service 返回结构化 patch 或 unavailable，不再由 UI 伪造。
...
目标：移除旧 preview / fallback 路径，让真实模块边界成为唯一产品路径。

* 删除 UI 本地伪造解释、比较、导出；
* PixiJS 成为核心呈现引擎；
* seed scene 与 source terrain 使用同一 12Level spine；
* 新 seed tab 必须进入 Electron tab runtime 与 workspace snapshot；
* 旧 workspace/runtime schema 不做向前兼容。

### P6：真实信息侦察与持久化

目标：引入外部信息，但仍保持结构可信。

* Playwright Scout service；
* 搜索结果抓取；
* source card；
* 来源去重；
* provenance metadata；
