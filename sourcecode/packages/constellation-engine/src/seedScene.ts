...
export function createSeedScene(seedText: string, options: SeedSceneOptions = {}): TerrainScene {
  ...
  const domainNodes: TerrainNode[] = domainTerms.map((term, index) => {
    ...
    return node({
      ...
      summary: `${term.canonical} is a configurable L0 domain entry. ${labelSummary ? `Language labels: ${labelSummary}.` : "No language labels configured."}`,
  ...
  const nodes: TerrainNode[] = [
    ...
    node({
      ...
      breadcrumb: [seed, "Concept neighborhood", "Context"],
      tabId,
      createdAt,
      canCreateSeed: true,
    }),
    node({
      ...
      breadcrumb: [seed, "Concept neighborhood", "Terms"],
      tabId,
      createdAt,
      canCreateSeed: true,
    }),
    node({
      ...
      position: { x: 0, y: -280 },
      parentId: ids.seed,
      breadcrumb: [seed, "Concept neighborhood", "Frontier"],
      tabId,
      createdAt,
    }),
    node({
      ...
      breadcrumb: [seed, "Source cluster", "Source intake"],
      tabId,
      createdAt,
    }),
    node({
    ...
    node({
      ...
      breadcrumb: [seed, "Phrase or term", phrase],
      tabId,
      createdAt,
      canCreateSeed: true,
    }),
    ...
    node({
      ...
      breadcrumb: [seed, "Character", character],
      tabId,
      createdAt,
      canCreateSeed: true,
    }),
    node({
      ...
      breadcrumb: [seed, "Unicode or dictionary", unicodeTitle],
      tabId,
      createdAt,
      canCreateSeed: true,
    }),
  ...
  return {
    ...
    metadata: {
      title: seed,
      description: `Local telescope seed for "${seed}". It has object-pool structure and L0-L10 depth, but no source-backed facts until Scout or source intake confirms material.`,
...
function createCanonicalSeedLayers(seed: string): TerrainLayer[] {
  const definitions = CANONICAL_LAYER_DEFINITIONS.filter((definition) => definition.order >= 0 && definition.order <= 10);
